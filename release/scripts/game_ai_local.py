...
4. confidence反映把握程度"""
    ...
    def __init__(
        ...
        try:
            ...
            if mode == "api":
                print("[警告] openai未安装: pip install openai")
    ...
    def _call_api(self, prompt: str, image_path: str = None, image_bytes: bytes = None) -> str:
        if not self._api_client:
            return '{"action":"wait","reasoning":"API客户端未初始化，请设置api-key"}'
...
def main():
    ...
    parser.add_argument("--api-model", default=os.environ.get("MODEL_NAME", "gpt-4o"), help="API模型名")
